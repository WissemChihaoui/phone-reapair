...
export default function FonctionCardView({ item, add, openAdd, deleteData, editData }) {
  const openDelete = useBoolean();
  const openEdit = useBoolean();

  const [nameField, setNameField] = useState(item?.label);
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        content={
          <>
            Êtes-vous sûr de vouloir supprimer la fonction <strong> {item?.label} </strong> ?
          </>
        }
        action={
          <Button
