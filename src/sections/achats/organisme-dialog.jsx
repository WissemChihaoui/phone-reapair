...
export default function OrganismeDialog({ open, onClose }) {
  ...
  React.useEffect(() => {
    if (table.page > 0 && organismeList.length <= table.page * table.rowsPerPage) {
  ...
  const handleAddOrganisme = () => {
    ...
    setOrganismeList([
      ...organismeList,
      {
        id: organismeList.length > 0 ? Math.max(...organismeList.map((o) => o.id)) + 1 : 1,
        name: newOrganisme,
      },
    ]);
    setNewOrganisme('');
  ...
  const handleEditSave = (id) => {
    setOrganismeList(
      organismeList.map((org) => (org.id === id ? { ...org, name: editValue } : org))
    );
    setEditId(null);
    setEditValue('');
  };
          ...
          <Table>
                ...
                <TableRow key={org.id}>
                  {editId === org.id ? (
                    <>
                      ...
                      <TableCell>
                        <IconButton color="primary" onClick={() => handleEditSave(org.id)}>
                    ...
                    <>
                      ...
                      <TableCell>
                        <IconButton
                          color="primary"
                          onClick={() => handleEditClick(org.id, org.name)}
                        >
                          <Iconify icon="tabler:edit" />
                        </IconButton>
                        <IconButton color="error" onClick={() => handleOpenDeleteDialog(org.id)}>
            ...
            <TableFooter>
              <TableRow>
                ...
                <TableCell>
                  <Button variant="contained" color="primary" onClick={handleAddOrganisme}>
                    Créer Organisme
                  </Button>
                </TableCell>
              </TableRow>
